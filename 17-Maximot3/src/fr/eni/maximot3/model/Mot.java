...
public class Mot {
	...
	public void reponse() {
		ArrayList<String> listeMotsPG = Dico.verifMots(mot);
		Iterator<String> it = listeMotsPG.iterator();
		int tailleMot = 2;
		System.out.println("Les mots de taille " + tailleMot);
		while (it.hasNext()) {
			String string = it.next();
			if (memeLettres(string)) {
				if (string.length() > tailleMot) {
					tailleMot=string.length();
					System.out.println("Les mots de taille " + tailleMot);
				}
